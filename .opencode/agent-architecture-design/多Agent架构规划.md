...
**回溯实现策略：**
- 每步产出物写入 `docs/` 目录作为文件快照
- 前端专家在修改代码前，先将原文件备份到 `docs/backups/`
- 回溯时项目经理调用前端专家从备份恢复文件
- 清除后续步骤的产出文件，重新执行目标步骤
- 更新 `~/.config/opencode/worker/process.md` 进度状态
...
权限配置 (permission):
  edit: { "~/.config/opencode/worker/process.md": "allow", "~/.config/opencode/worker/workflow.md": "allow", "docs/**": "allow", "*": "deny" }
  bash: allow               # 允许运行命令
  read: allow               # 允许读取文件
  task: allow               # 允许调用子 agent
  external_directory: { "~/.config/opencode/**": "allow", "src/**": "allow", "*": "ask" }
...
进度监控:
  每步执行前后，更新 ~/.config/opencode/worker/process.md (Markdown + YAML frontmatter 格式):
  ```markdown
  ---
  task: 任务描述
...
流程编排规则 (最高优先级):
  ...
  2. 读取 ~/.config/opencode/worker/process.md 确认当前步骤
...
输出物: PRD文档（Markdown格式，写入 docs/prd.md）
...
输出物: UI设计规范（Markdown + 组件树ASCII图，写入 docs/design.md）
...
备份清理规则:
  ...
  - 整个任务完成后，清理 docs/backups/ 目录中的所有文件
  - 用户取消任务时，清理所有备份文件
  - 回溯过程中不清理备份，回溯完成后再清理
...
| 文件路径 | 类型 | 说明 |
|---------|------|------|
| `~/.config/opencode/agents/project-manager.md` | Agent定义 | 项目经理角色（含完整 prompt） |
| `~/.config/opencode/agents/product-manager.md` | Agent定义 | 产品经理角色（含完整 prompt） |
| `~/.config/opencode/agents/ui-designer.md` | Agent定义 | UI设计师角色（含完整 prompt） |
| `~/.config/opencode/agents/frontend-expert.md` | Agent定义 | 前端专家角色（含完整 prompt） |
| `~/.config/opencode/agents/qa-engineer.md` | Agent定义 | 测试专家角色（含完整 prompt） |
| `~/.config/opencode/worker/process.md` | 进度文件 | 实时任务进度（Markdown + YAML frontmatter格式） |
| `~/.config/opencode/worker/workflow.md` | 流程定义文件 | 流程步骤定义（不可变，单一事实源） |
| `docs/` | 运行时目录 | 步骤产出物存储（动态生成） |
| `docs/backups/` | 备份目录 | 代码修改前备份（动态生成） |
    ...
    "project-manager": {
      ...
      "permission": {
        "edit": {
          ...
          "~/.config/opencode/worker/workflow.md": "allow",
          "docs/**": "allow",
          "*": "deny"
        },
        "bash": "allow",
...
根据 OpenCode 源码 (`loadAgent()` 函数)，`~/.config/opencode/agents/*.md` 文件解析规则：
...
### 流程文件
- 流程定义: ~/.config/opencode/worker/workflow.md（不可变，单一事实源）
- 运行时状态: ~/.config/opencode/worker/process.md（可变，每次执行后更新）
...
选择 [返回上一步] → 回到步骤3（UI设计师）
       ...
       ▼
项目经理执行回溯流程：
  1. 调用前端专家从 docs/backups/ 恢复代码
  2. 清除步骤4和步骤5的产出文件
  3. 更新 ~/.config/opencode/worker/process.md 进度状态
...
用户中途取消：
   保留已完成的产出物到 docs/
   保留进度状态到 ~/.config/opencode/worker/process.md
   下次直接对话即可从断点继续
```
...
```
项目经理实时更新 ~/.config/opencode/worker/process.md (Markdown + YAML frontmatter):
...
|------|-------|------|------|------|
| plan | project-manager | completed | 执行计划 | 2026-04-02 10:00 |
| prd | product-manager | completed | docs/prd.md | 2026-04-02 10:05 |
...
在 `~/.config/opencode/commands/` 中可添加：
...
```
1. 在 ~/.config/opencode/agents/ 中添加新的 .md 文件
2. 在项目经理 prompt 的职责清单中添加一行
3. 在 ~/.config/opencode/worker/process.md 的 agents.required 中添加新 agent
4. 无需修改项目经理的核心编排逻辑
```
...
```jsonc
// ~/.config/opencode/worker/agent-registry.json (可选)
{
  "agents": [
    {
...
项目经理是编排者，不应直接修改项目源代码文件。所有代码变更由前端专家执行，确保职责分离和流程可控。项目经理仅有 `~/.config/opencode/worker/process.md`、`~/.config/opencode/worker/workflow.md` 和 `docs/**` 的写权限用于更新进度和产出物，`edit` 权限末尾添加 `"*": "deny"` 兜底确保不会意外修改其他文件。
...
本项目不依赖 Git 操作。回溯通过文件备份机制实现：前端专家在修改代码前自动备份到 `docs/backups/`，回溯时从备份恢复。这使得项目可以在非 Git 仓库中正常工作。

### 11.8 为什么需要进度监控文件？

`~/.config/opencode/worker/process.md` 提供实时的任务状态追踪，支持：
- 用户随时了解当前进度
- 中断后可从断点继续
- 便于调试和问题排查
