...
> **🚨 强制约定：所有时间字段必须使用 `yyyy-mm-dd hh:mm:ss` 格式，时间为当前系统时间**
...
```json
{
  "startTime": "{当前时间 yyyy-mm-dd hh:mm:ss}",
  "taskName": "{任务名称}",
  "workflow": "prototype",
  "workflowVersion": 2,
  "agents": [
  ...
  "currentStep": "plan",
  "lastUpdate": "{当前时间 yyyy-mm-dd hh:mm:ss}"
}
```
...
| **用户确认** | 查找当前 step 对应的 todo，status 改为 completed；更新 currentStep 为下一步骤；标记下一 todo 为 in_progress |
...
| **用户确认** | user_gate 步骤用户确认后 | 更新 completed 和 current → **同步更新 agent_schedule.json: todos[当前步骤].status=completed, currentStep=下一步骤** |
...
**用户选项处理**：
- A → 更新 process.md，current = parallel_design_prd；**同步更新 agent_schedule.json: todos[user_gate_plan].status=completed, currentStep=parallel_design_prd；插入所有参与 agents（product-manager, ui-designer, frontend-expert, qa-engineer）**
...
**用户选项处理**：
- A → 更新 process.md，current = code；**同步更新 agent_schedule.json: todos[parallel_design_prd].status=completed, todos[user_gate_design_prd].status=completed, currentStep=code**
...
| 用户选择 | 处理逻辑 |
|----------|----------|
| A（确认） | 更新 process.md，current 指向下一步；**同步更新 agent_schedule.json: todos[gate步骤].status=completed, currentStep=下一步骤** |
...
> **禁止在任何文件中硬编码时间，所有时间必须使用系统命令动态获取**

### 获取当前时间的正确方式

```bash
# 正确：使用 date 命令获取当前系统时间
date "+%Y-%m-%d %H:%M:%S"
```

### 禁止行为
- ❌ 硬编码时间如 "2026-04-09 10:01"
- ❌ 使用示例时间如 "2026-01-01 00:00:00"
- ❌ 复制模板中的时间占位符而不替换

### 正确行为
- ✅ 创建/更新任何时间字段前，先执行 `date` 命令获取当前时间
- ✅ 时间格式必须为 `yyyy-mm-dd hh:mm:ss`（24小时制）
- ✅ agent_schedule.json 中的 startTime、lastUpdate、dispatchedAt、completedAt 必须动态获取

### 检查清单（每次更新前必须验证）
- [ ] 执行 `date "+%Y-%m-%d %H:%M:%S"` 获取当前时间
- [ ] 用获取的时间替换所有时间字段
- [ ] 确认没有残留的硬编码时间或示例时间
