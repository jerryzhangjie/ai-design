...
💼 产品经理 (subagent)
   │ 输入: 执行计划
   │ 输出: PRD文档（功能点+交互逻辑+验收标准）
   │ 边界: 只定义"做什么"和"怎么交互"，不涉及视觉和代码
   │ 实现: 被项目经理通过 task 工具调用，产出写入 .opencode/work/prd.md
   ▼
🎨 UI设计师 (subagent)
   │ 输入: PRD文档
   │ 输出: UI设计规范（组件树+布局+视觉规范）
   │ 边界: 只定义"长什么样"，不写代码
   │ 实现: 被项目经理调用，产出写入 .opencode/work/design.md
   ▼
💻 前端专家 (subagent)
   │ 输入: UI设计规范
   │ 输出: Vue组件文件 + 路由配置
   │ 边界: 严格按规范实现代码，不自行决策设计
   │ 实现: 被项目经理调用，直接修改项目文件
   ▼
🧪 测试专家 (subagent)
   │ 输入: 生成的代码
   │ 输出: 测试报告（构建结果+规范检查）
   │ 边界: 只验证不修改，发现问题交由前端专家修复
   │ 实现: 被项目经理调用，运行 npm run build 并检查
```
...
Steps: 20 (允许最多20次 agentic 迭代)
Color: primary

权限配置 (permission):
  edit: { ".opencode/worker/process.md": "allow" }
  write: { ".opencode/worker/process.md": "allow" }
  bash: allow               # 允许运行命令
  read: allow               # 允许读取文件
  task: allow               # 允许调用子 agent

模型选择: claude-sonnet-4-5（强推理和调度能力）
输出物: 执行计划清单（Markdown表格）+ 进度文件（JSON）
...
| `.opencode/commands/prototype.md` | 命令定义 | `/prototype` 快捷命令 |
| `.opencode/work/` | 运行时目录 | 步骤产出物存储（动态生成） |
| `.opencode/work/backups/` | 备份目录 | 代码修改前备份（动态生成） |
| `.opencode/worker/process.md` | 进度文件 | 实时任务进度（JSON格式） |
...
```bash
# 在TUI中输入
/prototype 做一个用户管理页面，有搜索、列表、分页
```

### 8.2 直接对话启动

```
用户: 帮我做一个设置页面，左边是导航菜单，右边是表单配置
       │
       ▼
🎯 项目经理自动接管 → 生成执行计划 → 等待确认 → 逐步执行
```
...
用户中途取消：
  ...
  下次可通过 /prototype 命令从断点继续
```

### 8.5 进度监控示例

```
项目经理实时更新 .opencode/worker/process.md:
...
项目经理是编排者，不应直接修改文件。所有代码变更由前端专家执行，确保职责分离和流程可控。项目经理仅有 `.opencode/worker/process.md` 的写权限用于更新进度。
...
`steps` 控制最大 agentic 迭代次数，防止 agent 陷入无限 tool call 循环。项目经理需要较多步骤（20）来编排整个流程，前端专家需要中等步骤（15）来完成代码生成。
